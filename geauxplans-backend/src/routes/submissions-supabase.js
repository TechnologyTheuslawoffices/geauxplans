 ...
 *
 * The older `USE_DOCTOOLS` flag is NOT honoured. Every .env in this repo sets
 * it to true, so treating it as a fallback would leave every environment on
 * doc-tools and make this switch a no-op. It is warned about instead of
 * silently ignored, so a stale variable is visible in the boot log.
 */
/*
...
if (process.env.USE_DOCTOOLS !== undefined && !process.env.DOC_ENGINE) {
  console.warn(
    `USE_DOCTOOLS is set but no longer read — documents are generated by "${DOC_ENGINE}". ` +
    'Set DOC_ENGINE=doctools to go back to the remote service, then remove USE_DOCTOOLS.'
  );
}
