...
import { Token, Formatter } from './doc-types';
...
function tokenizeCondition(condition: string): string[] {
  ...
  for (let i = 0; i < condition.length; i++) {
    ...
    // `|` and `:` added so pipe-filter syntax (`Var|contains: "X"`,
    // `Var|any: cond`) tokenizes into discrete tokens rather than
    // being glued into the preceding identifier.
    if (['(', ')', '!', '>', '<', '.', ',', '|', ':'].includes(char)) {
      if (current) {
        tokens.push(current);
        current = '';
...
export function parseCondition(condition: string): ConditionNode {
  ...
  function parsePropertyOrCall(): ConditionNode {
    ...
    // Knackly pipe-filter syntax: `<expr>|name` or `<expr>|name: <arg>`.
    // Translate into method-call AST so `evaluateFunctionCall` (which
    // already dispatches on `<path>.<method>`) handles the runtime semantics
    // for `contains`, `any`, `every`, `endsWith`, `startsWith`, etc.
    //
    // Arg-parsing precedence:
    //   - value-filters (contains, endsWith, startsWith, sort, format, else,
    //     punc): parseUnary — primary or `!primary`, NOT a full expression.
    //     This prevents `Var|contains: "X" && Other` from swallowing `Other`.
    //   - expression-filters (any, every, filter, map): parseOr — full
    //     boolean. Callers should paren-wrap when followed by `&&`/`||`.
    //
    // Chained pipes (`Var|filter: X|sort: Y`) emit a dotted name based on
    // the previous call's functionName; the evaluator's variable resolver
    // will return undefined for that path, so chained pipes degrade to
    // false rather than producing wrong results. The EstatePlanning
    // catalog gates use only single-pipe `|contains:` so this is acceptable.
    const VALUE_FILTERS = new Set([
      'contains', 'endsWith', 'startsWith',
      'sort', 'format', 'else', 'punc',
      'upper', 'lower', 'titlecaps', 'initcap',
      'cardinal', 'ordinal', 'keepsections',
    ]);
    while (peek() === '|') {
      consume(); // '|'
      const filterName = consume();
      const filterArgs: ConditionNode[] = [];
      if (peek() === ':') {
        consume(); // ':'
        filterArgs.push(VALUE_FILTERS.has(filterName) ? parseUnary() : parseOr());
      }
      const objName =
        node.variable ??
        (node as ConditionNode & { functionName?: string }).functionName ??
        '';
      const dottedName = objName ? `${objName}.${filterName}` : filterName;
      node = { type: 'call', functionName: dottedName, args: filterArgs };
    }
...
export function buildTemplateAst(tokens: Token[]): TemplateNode {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    switch (token.type) {
      ...
      case 'elseif': {
        // If we're currently inside a previous elseif container, pop it so
        // the new branch is registered on the parent if node itself.
        if (
          stack.length > 1 &&
          stack[stack.length - 1].type === 'root' &&
          stack[stack.length - 2]?.type === 'if'
        ) {
          stack.pop();
        }
      ...
      case 'else': {
        // If we're inside an elseif container, pop it so the else branch
        // is registered on the if node itself.
        if (
          stack.length > 1 &&
          stack[stack.length - 1].type === 'root' &&
          stack[stack.length - 2]?.type === 'if'
        ...
        if (parent.type === 'if') {
          parent.elseChildren = [];
          const elseContainer: TemplateNode = { type: 'root', children: parent.elseChildren };
      ...
      case 'endif':
        // Pop any open else/elseif container (a 'root' wrapper) above the if,
        // then pop the if node itself. Critically: only pop ONE if level so
        // that a nested {[if]}…{[endif]} doesn't close its parent.
        while (
          stack.length > 1 &&
          stack[stack.length - 1].type !== 'if'
        ) {
          // Bail out if we'd cross a list boundary (shouldn't happen with
          // well-formed templates but guards against malformed input).
          if (stack[stack.length - 1].type === 'list') break;
          stack.pop();
        }
