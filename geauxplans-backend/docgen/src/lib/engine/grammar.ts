...
import { parsePipe } from './expr';
import type { BlockNode, Paragraph, RunFmt, Expr, Filter } from './types';
...
function parseIfBlock(items: FlatItem[], cursor: { i: number }, rPr: RunFmt, condText: string): BlockNode {
  ...
  // Unclosed — exhausted items without finding {[endif]}.
  // groupParagraphsByBalance now synthesizes matching closers at EOF for
  // truly-dangling opens, so reaching this point means the parser walked
  // off the end without seeing one (a deeper-nested block consumed it).
  // Returning the partially-built `if` block is the SAFE fallback: the
  // resolver evaluates the condition normally — true emits the then-body,
  // false suppresses it — and no literal `{[if ...]}` text ever leaks
  // into the output. The legacy cap-and-rewind path emitted the directive
  // as literal text, which produced visible leaks; we drop it entirely.
  if (phase === 'elseif' && curEliCond !== null) {
    elseifs.push({ cond: curEliCond, body: curBucket });
  }
  return { kind: 'if', rPr, cond, then: thenBlocks, elseifs, else: elseBlocks };
...
function parseListBlock(items: FlatItem[], cursor: { i: number }, rPr: RunFmt, sourceText: string): BlockNode {
  ...
  // Unclosed — same EOF-fallback behavior as parseIfBlock. Return the
  // partially-built list block; the resolver iterates over whatever source
  // resolves to (empty → emits nothing; non-empty → emits the body N times).
  // Never leaks as literal text.
  return { kind: 'list', rPr, source, filters, body };
}
