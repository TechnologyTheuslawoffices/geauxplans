...
import { neutralizeHyperlinkStyle } from './xml';
import {
  NORMALIZER_VERSION,
...
export async function normalizeDocx(buffer: Uint8Array): Promise<NormalizationResult> {
  const stats: NormalizationStats = {
    ...
    hyperlinkStylesNeutralized: 0,
  };

  const doc = loadDocument(buffer);

  // F1: make TOC entries render plain black like Real Knackly by neutralizing
  // the blue+underline Hyperlink character style (estate docs emit only
  // internal _Toc anchors, so this affects exactly the Table of Contents).
  if (doc.stylesXml) {
    const h = neutralizeHyperlinkStyle(doc.stylesXml);
    if (h.changed) {
      doc.stylesXml = h.stylesXml;
      stats.hyperlinkStylesNeutralized++;
    }
  }
