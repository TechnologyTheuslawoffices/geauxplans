...
export function consolidateParagraph(
  ...
  // Strip designer highlight from the paragraph-mark run properties
  // (<w:pPr><w:rPr>…</w:rPr></w:pPr>). These style the pilcrow / empty-paragraph
  // shading and are NOT collected as <w:r> run blocks below, so they'd otherwise
  // leak the designer blue/green/orange color + shading into the output.
  let working = paragraphInner;
  const pPrMatch = working.match(/<w:pPr>[\s\S]*?<\/w:pPr>/);
  if (pPrMatch) {
    const pPrBlock = pPrMatch[0];
    const markRPr = pPrBlock.match(/<w:rPr\b[^>]*>[\s\S]*?<\/w:rPr>|<w:rPr\b[^>]*\/>/);
    if (markRPr) {
      const { rPr: cleaned, stripped } = stripDesignerHighlight(markRPr[0]);
      if (stripped) {
        stats.designerHighlightsStripped++;
        working = working.replace(pPrBlock, pPrBlock.replace(markRPr[0], cleaned));
      }
    }
  }

  const runs = collectRunBlocks(working);
  if (runs.length === 0) return { xml: working, stats };
