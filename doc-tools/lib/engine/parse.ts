  ...
  while (i < bodyInner.length) {
    ...
    if (bodyInner.startsWith('<w:p', i) && (bodyInner[i + 4] === ' ' || bodyInner[i + 4] === '>')) {
      ...
      if (bodyInner[tagEnd - 1] === '/') {
        ...
        blocks.push({
          kind: 'paragraph',
          para: makeEmptyParagraph(bodyInner.substring(i, tagEnd + 1)),
        });
        i = tagEnd + 1;
        continue;
      }
...
function parseParagraph(openTag: string, inner: string): Paragraph {
  ...
  // Find first <w:r>; everything before that (other than pPr) is preRunXml.
  const firstRun = findRunStart(afterPPr);
  let preRunXml = '';
  let runsRegion = afterPPr;
  if (firstRun >= 0) {
    preRunXml = afterPPr.substring(0, firstRun);
    runsRegion = afterPPr.substring(firstRun);
  } else {
    preRunXml = afterPPr;
    runsRegion = '';
  }
  ...
  let i = 0;
  let lastRunEnd = 0;
  while (i < runsRegion.length) {
    const rs = findRunStart(runsRegion.substring(i));
    if (rs === -1) break;
    const absRs = i + rs;
    const tagEnd = runsRegion.indexOf('>', absRs);
    if (tagEnd === -1) break;
    if (runsRegion[tagEnd - 1] === '/') {
      i = tagEnd + 1;
      continue;
    }
    const closeStart = findClosingTag(runsRegion, tagEnd + 1, 'w:r');
    if (closeStart === -1) break;
    const runInner = runsRegion.substring(tagEnd + 1, closeStart);
    runs.push(parseRun(runInner));
    lastRunEnd = closeStart + '</w:r>'.length;
    i = lastRunEnd;
  }

  const postRunXml = runsRegion.substring(lastRunEnd);
