...
export function materializeParagraph(p: Paragraph, spans: ResolvedSpan[]): string {
  ...
  return `${p.openTag}${p.pPr.raw}${p.preRunXml}${runsXml}${p.postRunXml}</w:p>`;
}
...
export function materializeGroup(paras: Paragraph[], spans: ResolvedSpan[]): string {
  ...
  const flush = () => {
    if (bucket.length === 0) {
      ...
      const hasDirective = originalText.includes('{[');
      const hasNoContent = originalText.trim() === '' || originalText.replace(/\{[^}]+\}/g, '').trim() === '';
      // Preserve if: no directives AND no visible content (intentionally empty for spacing)
      if (!hasDirective && hasNoContent) {
        xml += `${srcPara.openTag}${srcPara.pPr.raw}${srcPara.preRunXml}${srcPara.postRunXml}</w:p>`;
      }
      return;
    }
